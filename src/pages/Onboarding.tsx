...
const Onboarding = () => {
  ...
  const [showNtrp, setShowNtrp] = useState(false);
  const [pendingNtrp, setPendingNtrp] = useState<number | null>(null);
  ...
  const completeOnboarding = async () => {
    try {
      ...
      console.error('Error completing onboarding:', error, JSON.stringify(error));
      toast({
        title: "Error",
        description: error?.message || JSON.stringify(error) || "Failed to complete profile setup. Please try again.",
        variant: "destructive",
      });
    }
  };
    ...
    switch (currentStep) {
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div>
              <Label htmlFor="location" className="text-base font-medium text-gray-900">ZIP Code</Label>
              <Input
                id="location"
                type="text"
                inputMode="numeric"
              ...
              {formData.location && formData.location.length !== 5 && (
                <p className="text-red-500 text-sm mt-2">ZIP code must be exactly 5 digits.</p>
              )}
            </div>
          </div>
        );
      ...
      case 4:
        if (formData.ratingMethod === "quiz") {
          if (showNtrp && pendingNtrp !== null) {
            return (
              <div className="flex flex-col items-center justify-center space-y-8">
                <div className="text-center">
                  <h2 className="font-bricolage text-3xl font-bold text-gray-900 mb-4">Your NTRP Rating</h2>
                  <div className="text-6xl font-extrabold text-emerald-600 mb-2">{pendingNtrp.toFixed(1)}</div>
                  <p className="text-lg text-gray-700 mb-6">This is your estimated NTRP rating based on your quiz answers.</p>
                  <Button className="gradient-primary text-white px-8 py-4 text-lg font-semibold" onClick={() => { setShowNtrp(false); setPendingNtrp(null); handleNext(); }}>
                    Continue
                  </Button>
                </div>
              </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <h2 className="font-bricolage text-3xl font-bold text-gray-900 mb-2">When do you usually play?</h2>
              <p className="text-gray-600 text-lg">Select the option that best matches your schedule</p>
            </div>
            <div className="space-y-4">
              {(
                [
                  { value: "hard_at_work", label: "Hard at Work", desc: "I work full-time, so I play evenings and weekends." },
                  { value: "student", label: "Student", desc: "I'm a student, so I'm available evenings and weekends." },
                  { value: "parent", label: "Parent", desc: "I'm available during school hours and weekends." },
                  { value: "flexible", label: "Flexible", desc: "My schedule is flexible, I can play most times." },
                  { value: "early_bird", label: "Early Bird", desc: "I prefer to play early mornings." },
                  { value: "night_owl", label: "Night Owl", desc: "I prefer to play late evenings or nights." },
                  { value: "weekend_warrior", label: "Weekend Warrior", desc: "I'm mostly available on weekends." },
                ]
              ).map(opt => (
                <div
                  key={opt.value}
                  className={`flex items-center space-x-3 p-6 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-emerald-500 hover:bg-emerald-50/30 ${
                    formData.availability === opt.value ? 'border-emerald-500 bg-emerald-50/50' : 'border-gray-200'
                  }`}
                  onClick={() => updateFormData('availability', opt.value)}
                >
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all duration-200 ${
                    formData.availability === opt.value ? 'border-emerald-500 bg-emerald-500' : 'border-gray-300'
                  }`}>
                  ...
                  <div className="flex-1">
                    <Label className="font-semibold text-gray-900 cursor-pointer text-lg">{opt.label}</Label>
          ...
          <div className="flex justify-between items-center mt-8 max-w-2xl mx-auto">
            ...
            <Button
              ...
              disabled={
                ...
                (currentStep === 2 && (!formData.location || formData.location.length !== 5)) ||
